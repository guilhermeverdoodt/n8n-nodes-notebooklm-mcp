...
class APIKeyMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        api_key = os.environ.get("NOTEBOOKLM_API_KEY")
        
        # Ignora se for a rota de health check
        if request.url.path == "/health":
            return await call_next(request)

        # Se a chave não estiver configurada no ambiente, desabilita a segurança
        if not api_key:
            return await call_next(request)
            
        # Verifica o cabeçalho X-API-Key
...
async def rest_list_notebooks(request: Request) -> JSONResponse:
    """REST endpoint to list notebooks without JSON-RPC."""
    from .tools.notebooks import notebook_list
    try:
...
async def rest_query(request: Request) -> JSONResponse:
    ...
    try:
        ...
        result = notebook_query(notebook_id=notebook_id, query=query)
        return JSONResponse(result)
    except Exception as e:
        return JSONResponse({"error": str(e)}, status_code=500)
